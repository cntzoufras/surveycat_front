...
export const fetchQuotesBySymbolRequest = createAction('FETCH_QUOTES_BY_SYMBOL_REQUEST');
export const fetchQuotesBySymbolSuccess = createAction('FETCH_QUOTES_BY_SYMBOL_SUCCESS');
export const fetchQuotesBySymbolError = createAction('FETCH_QUOTES_BY_SYMBOL_ERROR');
  ...
  try {
    ...
    dispatch(
      fetchQuotesBySymbolError(
        firstLetterToUpperCase(e.response?.data?.error || e.response.statusText),
      ),
    );
  }
};
